...
public interface IValueService
{
    ...
    public Task<(List<OutValueDto>?, ErrorBase?)> GetListAsync(long appId, long languageId, string? search, long? lastId);
    public Task<(OutValueDto?, ValidationResult?, ErrorBase?)> AddAsync(long appId, long languageId, InValueDto dto);
    public Task<(OutValueDto?, ValidationResult?, ErrorBase?)> UpdateAsync(long appId, long languageId, long id, InValueDto dto);
