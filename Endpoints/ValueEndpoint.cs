...
public class ValueEndpoint : IEndpoints
{
    ...
    private static async Task<IResult> GetAsync(long appId, long languageId, long id, ValueService service)
    {
        var result = await service.GetAsync(appId, languageId, id);
        return result.Response();
    }

    private static async Task<IResult> AddAsync(long appId, long languageId, InValueDto dto, ValueService service)
    {
    ...
    private static async Task<IResult> DeleteAsync(long appId, long languageId, long id, ValueService service)
    {
